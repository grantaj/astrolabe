...
def _mount() -> IndiMountBackend:
    ...
    mount._client = MagicMock()
    return mount


def test_tracking_disable_requires_track_off_switch():
    mount = _mount()
    mount._client.has_prop.return_value = False

    with pytest.raises(BackendError, match="TRACK_OFF is unavailable"):
        mount.set_tracking(False)

    mount._client.setprop.assert_not_called()


def test_tracking_disable_waits_for_reported_off_state():
    mount = _mount()
    mount._client.has_prop.return_value = True
    ...
    mount._client.setprop.assert_called_once_with(
