...
class SolarSystemProvider(CatalogProvider):
    ...
    def list_targets(self):
        raise NotImplementedError(
            "Use list_solar_system_targets(window_start_utc, window_end_utc)"
        )
...
def _planet_ra_dec(
    planet: str, d: float, earth_xyz: tuple[float, float, float]
...
def _solve_kepler(m: float, e: float) -> float:
    e_anom = m
    for _ in range(8):
        e_anom = e_anom - (e_anom - e * math.sin(e_anom) - m) / (
            1 - e * math.cos(e_anom)
        )
    return e_anom
