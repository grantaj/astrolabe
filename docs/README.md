...
2. **Living repository documentation** defines intended current contracts, invariants, and reference behaviour.
...
- `pointing_model.md` — current offset-only pointing model and tangent-plane convention.
- `planner_visibility.md` — current planner visibility-scoring rationale.

Focused references exist only when current behaviour or rationale would be awkward or noisy in the core documents.

## Planning policy

Do not add implementation plans, roadmaps, status snapshots, or completed-plan archives under `docs/`.

Future work belongs in GitHub issues, where dependencies, acceptance criteria, discussion, and lifecycle state can remain visible. Once work is implemented, the issue/PR and git history provide the historical record; update living docs only where the implemented contract, invariant, or useful current rationale changed.

Before deleting a stale plan, classify every still-valid part:

- current behaviour -> code/tests and, if useful, a living reference;
- future work -> an open issue;
- historical rationale -> git/merged PR history;
- superseded intent -> deliberately discard it.

Do not use git history as a substitute for unimplemented future work. The point of hygiene is to remove ambiguity, not knowledge.
