...
`FocusGuidanceEstimator` consumes only fresh valid HFR history and returns focus-owned temporal states: `unknown`, `improving`, `best-observed`, or `worsening`. These states describe image-quality evidence in the user's current motion; they are deliberately not signed focuser corrections. `best-observed` requires improvement followed by worsening to bracket a local best in the observed temporal path, then a stable return near that bracketed best. A substantially better newly observed HFR invalidates the old bracket and must itself be bracketed. Invalid measurements or stale history reset the inference state.
