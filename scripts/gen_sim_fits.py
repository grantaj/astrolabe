...
def main() -> int:
    ...
    ap.add_argument(
        "--count", type=int, default=3, help="Number of FITS frames to generate"
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--outdir",
        type=Path,
        default=Path("testdata/raw"),
        help="Output directory for FITS",
    )
    ap.add_argument(
        "--prefix",
        type=str,
        default="astrolabe_sim_",
        help="Output filename prefix for numbered copies",
    )
    ap.add_argument(
        "--settle",
        type=float,
        default=0.3,
        help="Extra seconds to wait after each exposure update",
    )
    ap.add_argument(
        "--guider",
        action="store_true",
        help="Use GUIDER_EXPOSURE instead of CCD_EXPOSURE (smaller frames)",
    )
    ...
    setprop(
        host,
        port,
        f"{DEVICE}.UPLOAD_SETTINGS.UPLOAD_DIR",
        str(outdir.resolve()),
        soft=True,
    )
    setprop(
        host, port, f"{DEVICE}.UPLOAD_SETTINGS.UPLOAD_PREFIX", args.prefix, soft=True
    )
    ...
    print(
        f"[info] Generating {args.count} frame(s) at {args.exposure:.2f}s ({'GUIDER' if args.guider else 'CCD'})"
    )
    ...
    for i in range(args.count):
        ...
        prev_mtime = wait_for_mtime_increase(
            base_path, prev_mtime, timeout_s=max(10.0, args.exposure + 5.0)
        )
